...
# 4. FILE UPLOADER & MAPPING PRIORITY
...
if uploaded_file:
    try:
        ...
        # 🎯 SMART MAPPING: Prioritize Detail/Description over ID
        name_map = {}
        # First Pass: Find the descriptive Item Name
        for c in df.columns:
            if any(x in c for x in ['detail', 'description']):
                name_map[c] = 'item'
                break
        # Second Pass: If no "detail" found, look for product/item but AVOID ID
        ...
        # Keep only the necessary columns to avoid duplicate confusion
        ...
        if all(col in df.columns for col in ['item', 'quantity', 'price']):
            ...
            if 'date' in df.columns:
                ...
                if not valid_dates.empty:
                    ...
                    # For Maven, this should result in ~5.9
                    months_in_data = max(1.0, days_span / 30.44)
    except Exception as e: st.error(f"File Error: {e}")

# 5. PRICING ENGINE WITH NORMALIZED MATH
if df is not None:
    # Summarize lifetime units
    summary = df.groupby('item').agg({'quantity': 'sum', 'price': 'mean'}).reset_index()
    
    # 1. Force normalized units for the monthly calculation
    ...
    def run_optimization(row):
        ...
        if m_units > 35:
            ...
            # Math strictly uses monthly normalized units
    ...
    # 2. Metric sums only the normalized impacts
